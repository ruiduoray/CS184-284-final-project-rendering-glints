...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // Transforming the coordinate in the image into camera space
  double tan_hFov = tan(0.5 * (this->hFov * M_PI / 180));
  double tan_vFov = tan(0.5 * (this->vFov * M_PI / 180));
  Vector2D bottomLeft = Vector2D(-1 * tan_hFov, -1 * tan_vFov);
  Vector2D topRight = Vector2D(tan_hFov, tan_vFov);
  double width = topRight.x - bottomLeft.x;
  double height = topRight.y - bottomLeft.y;
  double x_cam = bottomLeft.x + x * width;
  double y_cam = bottomLeft.y + y * height;

  // Generate the ray in the camera space
  Vector3D d = Vector3D(x_cam, y_cam, -1);
  d.normalize();
  Ray ray_cam = Ray(pos, d);
  ray_cam.min_t = nClip;
  ray_cam.max_t = fClip;

  // Calculate the 4x4 camera-to-world transform matrix
  Matrix4x4 c2w4d;
  c2w4d[0] = Vector4D(this->c2w[0], pos.x);
  c2w4d[1] = Vector4D(this->c2w[1], pos.y);
  c2w4d[2] = Vector4D(this->c2w[2], pos.z);
  c2w4d[3] = Vector4D(0, 0, 0, 1);

  ray_cam.d = this->c2w * ray_cam.d;
  ray_cam.d.normalize();
